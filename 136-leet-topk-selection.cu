...
extern "C" void solve(float* input, float* output, int N, int k) {
  ...
  thrust::sort(input_ptr, input_ptr + N, thrust::greater<float>());
  thrust::copy(input_ptr, input_ptr + k, output_ptr);
}
