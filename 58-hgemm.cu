...
int main(int argc, char** argv) {
  ...
  // Initialize
  for (int i = 0; i < m * k; ++i) h_A[i] = static_cast<T>(static_cast<float>(rand()) / RAND_MAX);
  for (int i = 0; i < k * n; ++i) h_B[i] = static_cast<T>(static_cast<float>(rand()) / RAND_MAX);
  ...
  const int thread_block_swizzle_stride = 2048; 
  printf("ALGO = CuTe HGEMM, TN, STAGES=2, SMEM SWIZZLE=<3, 3, 3>, BLOCK SWIZZLE=%d\n", thread_block_swizzle_stride);

  // Warmup
  launch_hgemm_mma_stages_block_swizzle_tn_cute<T, 2, true>(d_A, d_B, d_C, m, n, k, thread_block_swizzle_stride);
  ...
  printf("Time = %f ms, ", avg_time_ms);
  printf("Performance = %10.4lf Tflops\n", tflops);
