...
__global__ void matrix_multiplication_kernel(const float* __restrict__ A,
  ...
  for (int bk = 0; bk < k_tiles; ++bk) {
    const int load_a_gemm_k = bk * BK + load_a_smem_k;
    const int load_b_gemm_k = bk * BK + load_b_smem_k;
    ...
    for (int i = 0; i < threads_per_row_load_a; i++) {
      if ((load_a_gemm_m < M) && (load_a_gemm_k + i < K)) {
        s_a[load_a_smem_m][load_a_smem_k + i] = A[load_a_gemm_addr + i];
      } else {
        s_a[load_a_smem_m][load_a_smem_k + i] = 0.0f;
      }
    }
    for (int i = 0; i < threads_per_row_load_b; i++) {
      if ((load_b_gemm_k < K) && (load_b_gemm_n + i < N)) {
        s_b[load_b_smem_k][load_b_smem_n + i] = B[load_b_gemm_addr + i];
      } else {
        s_b[load_b_smem_k][load_b_smem_n + i] = 0.0f;
      }
    }
    __syncthreads();
