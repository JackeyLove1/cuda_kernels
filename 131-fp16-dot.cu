 ...
 */
__forceinline__ __device__ float dot(const uint32_t a, const uint32_t b) {
  const uint16_t a_lo = static_cast<uint16_t>(a & 0xFFFFu);
  const uint16_t a_hi = static_cast<uint16_t>((a >> 16) & 0xFFFFu);
  const uint16_t b_lo = static_cast<uint16_t>(b & 0xFFFFu);
  const uint16_t b_hi = static_cast<uint16_t>((b >> 16) & 0xFFFFu);
  const half ha_lo = __ushort_as_half(a_lo);
  const half ha_hi = __ushort_as_half(a_hi);
  const half hb_lo = __ushort_as_half(b_lo);
  const half hb_hi = __ushort_as_half(b_hi);
  return __half2float(ha_lo) * __half2float(hb_lo) + __half2float(ha_hi) * __half2float(hb_hi);
}
__forceinline__ __device__ float dot(const uint4& a, const uint4& b) {
  float result = 0.0f;
  result += dot(a.x, b.x);
  result += dot(a.y, b.y);
  result += dot(a.z, b.z);
