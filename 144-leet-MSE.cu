...
/**
Implement a GPU program to calculate the Mean Squared Error (MSE) between predicted values and
target values. Given two arrays of equal length, predictions and targets, compute: MSE=1/N∑N where N
is the number of elements in each array.
 */
__global__ void mse_kernel(const float* __restrict__ predictions, const float* __restrict__ targets,
                           float* __restrict__ mse, int N) {
  const auto tid = threadIdx.x;
  const auto gid = blockDim.x * blockIdx.x + tid;
  const auto stride = blockDim.x * gridDim.x;
  const float4* p4 = LOAD4(predictions);
  const float4* t4 = LOAD4(targets);
  const auto vec_size = N >> 2;
  const auto vec_tail = N & 3;
  float sum{0.0f};
  ...
  if (vec_tail && gid < vec_tail) {
    const auto base = vec_size << 2;
    const auto idx = base + gid;
    auto diff = predictions[idx] - targets[idx];
    sum = fmaf(diff, diff, sum);
  }
  sum = BlockReduceDynamic(sum, ReduceOpType::SUM);
  if (tid == 0) {
    atomicAdd(mse, sum / static_cast<float>(N));
  }
}
...
extern "C" void solve(const float* predictions, const float* targets, float* mse, int N) {
  ...
  const auto blocksPerGrid = CEIL(N, threadsPerBlock * 4);
  mse_kernel<<<blocksPerGrid, threadsPerBlock>>>(predictions, targets, mse, N);
