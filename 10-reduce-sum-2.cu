...
__global__ void reduceSumKernel(const float *__restrict__ input, float *__restrict__ output, const int N)
{
    ...
    const auto              globalId = threadIdx.x + blockDim.x* blockIdx.x;

    sdata[tid] = input[globalId] + input[globalId + blockDim.x];
    __syncthreads();
...
extern "C" void solve(const float *input, float *output, int N)
{
    ...
    auto          blocksPerGrid   = CDIV(N, threadsPerBlock);
...
/**
Time: 0.091136 ms
Bandwidth: 43.910110 GB/s
Input size: 1000000
GPU Result: 4949860.000000
