...
/**
Implement a batched matrix multiplication in FP16. Given a batch of matrices A of shape [B, M, K]
and a batch of matrices B of shape [B, K, N], compute the output batch C of shape [B, M, N] such
that for each batch index b: Cb=Ab×Bb All matrices are stored in row-major order and use 16-bit
floating point numbers (FP16/ half). Accumulation during multiplication should use FP32 for better
precision before converting the final result to FP16.
 */

// A [M, K] B [K, N]
template <int TILE, int TM, int TN>
__global__ void kernel(const half* A, const half* B, half* C, int BATCH, const int M, const int N,
                       const int K) {
  ...
  for (int t = 0; t < num_tiles; ++t) {
    for (int r = 0; r < TM; ++r) {
      for (int c = 0; c < TN; ++c) {
        ...
        As[t_row][t_col] = (row0 + r < M && a_col + c < K)
                               ? __half2float(A[batch * M * K + (row0 + r) * K + a_col + c])
                               : 0.0f;
        Bs[t_row][t_col] = (col0 + c < N && b_row + r < K)
                               ? __half2float(B[batch * K * N + (b_row + r) * N + col0 + c])
                               : 0.0f;
      }
    }
    __syncthreads();
  ...
  for (int r = 0; r < TM; r++) {
    for (int c = 0; c < TN; c++) {
      if (row0 + r < M && col0 + c < N) {
        C[(batch * M * N) + (row0 + r) * N + col0 + c] = __float2half(acc[r][c]);
      }
    }
  }
}
