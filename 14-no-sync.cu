...
__global__ void sumKernel(const float *__restrict__ input, float *__restrict__ output, const int N)
{
    ...
    const int warpId    = tid >> 5;          // 修正拼写 wrap -> warp
...
extern "C" void solve(const float *input, float *output, int N)
{
    ...
    // 既然有了 Grid-Stride Loop，blocksPerGrid 可以固定或根据 GPU 利用率设置
    // 不需要再去为了覆盖 N 而计算非常大的 block 数
    constexpr int maxBlocks = 256; // 通常几百个 block 足以占满 GPU SM

    // 依然可以根据 N 动态调整，但要有上限
    int blocksPerGrid = std::min(maxBlocks, CDIV(N, threadsPerBlock * 4));

    sumKernel<<<blocksPerGrid, threadsPerBlock>>>(input, output, N);
    // 注意：atomicAdd 是必要的，因为每个 block 都会贡献部分和到 output
    // 这里没有清零 output，假设调用者已清零或 output 本就是累加目标
...
/**
Time: 0.080896 ms
Bandwidth: 49.468357 GB/s
Input size: 1000000
GPU Result: 4949858.000000
