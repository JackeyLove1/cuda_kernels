...
__global__ __launch_bounds__(256) void row_softmax_kernel(float* __restrict__ A, const int M,
  ...
  for (int i = tid; i < N; i += nthreads) {
    const float logit = A[row_offset + i] + alpha * static_cast<float>(row - i);
  ...
  for (int i = tid; i < N; i += nthreads) {
    const float logit = A[row_offset + i] + alpha * static_cast<float>(row - i);
    sum_val += __expf(logit - global_max);
  }
  ...
  for (int i = tid; i < N; i += nthreads) {
    const float logit = A[row_offset + i] + alpha * static_cast<float>(row - i);
    A[row_offset + i] = __expf(logit - global_max) / global_sum;
  }
}
...
extern "C" void solve(const float* Q, const float* K, const float* V, float* output, int M, int N,
  ...
  // const dim3 alibi_grid(CEIL(N, MATMUL_TILE), CEIL(M, MATMUL_TILE));
  // add_alibi_bias_kernel<<<alibi_grid, matmul_block>>>(scores, M, N, alpha);
  // CUDA_CHECK(cudaGetLastError());

  row_softmax_kernel<<<M, 256>>>(scores, M, N, alpha);
  CUDA_CHECK(cudaGetLastError());
