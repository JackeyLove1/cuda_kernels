...
template <auto BM=128, auto BN=128, auto BK=64, auto BP=3, class Alpha, class Beta>
void
        ...
        cudaStream_t stream = 0)
{
  ...
  // RTX 3080 (SM86) max shared memory is 100KB.
  ...
  if (thread0() && block0())
  {
    print("swizzle atmo: \n"); print(swizzle_atom);print("\n========\n");
    print("swizzle layout: \n"); print_layout(swizzle_atom);print("\n==========\n");
  }
#endif
...
int main(int argc, char** argv)
{
  ...
  // Timing iterations
    auto run_config = [&](auto BM, auto BN, auto BK, auto BP, const std::string& name) {
        constexpr int bm = decltype(BM)::value;
        constexpr int bn = decltype(BN)::value;
        constexpr int bk = decltype(BK)::value;
        constexpr int bp = decltype(BP)::value;

        timer.start();
        for (int i = 0; i < timing_iterations; ++i) {
            gemm_tn<bm, bn, bk, bp, cute::half_t, cute::half_t>(m, n, k,
                 alpha,
                 d_A.data().get(), ldA,
                 d_B.data().get(), ldB,
                 beta,
        ...
        CUTE_CHECK_LAST();
        std::cout << "Config: " << name <<" BM: " << bm << " BN:" << bn << " BK:"<< bk << " BP:"<< bp << std::endl;
