...
__global__ void matrix_multiplication_kernel(const float* __restrict__ A,
  ...
  auto async_load_tile = [&](int stage, int tile_k) {
    ...
    // Producer: global -> shared (cp.async under the hood)
  ...
  if (k_tiles > 0) {
    ...
    for (int bk = 0; bk < k_tiles; ++bk) {
      const int read_stage = bk & 1;
      const int next_tile = bk + 1;
      if (next_tile < k_tiles) {
        const int write_stage = next_tile & 1;
      ...
      // Consumer: compute from current stage
#pragma unroll
      for (int k = 0; k < BK; ++k) {
#pragma unroll
        for (int m = 0; m < TM; ++m) {
