...
/**
Implement a GPU program that compute s the softmax attention operation for a given set of matrices.
Given the query matrix Q of size M×d, key matrix K of size N×d, and value matrix V of size N×d, your
program should compute the output matrix using the formula: Attention(Q,K,V)=softmax(QKTV)V, where
the softmax function is applied row-wise. Implementation Requirements ·Use only GPU native features
(external libraries are not permitted) ·The solve function signature must remain unchanged ·The
final result must be stored in the output matrix output
 */
...
// inplace operation, add casual mask
__global__ __launch_bounds__(256) void row_softmax_kernel(float* __restrict__ A, const int M,
  ...
  __shared__ double global_sum;
  for (int i = tid; i < N; i += nthreads) {
    const float logits = (i <= row) ? A[row_offset + i] : -INFINITY;
    max_val = fmaxf(max_val, logits);
  }
  ...
  for (int i = tid; i < N; i += nthreads) {
    if (i <= row) {
      sum_val += exp(static_cast<double>(A[row_offset + i] - global_max));
    }
  }
  sum_val = BlockReduceOp<double, SumOp<double>>(sum_val);
  if (tid == 0) {
    global_sum = sum_val;
  }
  __syncthreads();
  ...
  for (int i = tid; i < N; i += nthreads) {
    if (i <= row) {
      const double numer = exp(static_cast<double>(A[row_offset + i] - global_max));
      A[row_offset + i] = static_cast<float>(numer / global_sum);
    } else {
      A[row_offset + i] = 0.0f;
    }
  }
...
extern "C" void solve(const float* Q, const float* K, const float* V, float* output, int M, int d) {
  ...
  const auto d_inv = 1.0f / sqrtf(d);
  float* scores = nullptr;
  const size_t scores_bytes = static_cast<size_t>(M) * static_cast<size_t>(N) * sizeof(float);
  CUDA_CHECK(cudaMalloc(&scores, scores_bytes));

  const dim3 matmul_block(MATMUL_TILE, MATMUL_TILE);
  const dim3 qk_grid(CEIL(N, MATMUL_TILE), CEIL(M, MATMUL_TILE));
  matmul_kernel<MATMUL_TILE><<<qk_grid, matmul_block>>>(Q, K, scores, M, d, N, 1.0f / d_inv);
  CUDA_CHECK(cudaGetLastError());

  row_softmax_kernel<<<M, 256>>>(scores, M, N, 0.0f);
  CUDA_CHECK(cudaGetLastError());

  const dim3 ov_grid(CEIL(d, MATMUL_TILE), CEIL(M, MATMUL_TILE));
  // scores[M, N] @ V[N, d] -> output[M, d]
  matmul_nn_kernel<MATMUL_TILE><<<ov_grid, matmul_block>>>(scores, V, output, M, N, d, 1.0f);
  CUDA_CHECK(cudaGetLastError());
