...
__forceinline__ __device__ int find_best_centroid(const float2 pt, const float2* s_centroid,
                                                  const int k) {
...
__global__ void kmeans_fused_kernel(const float* data_x, const float* data_y,
  ...
  int tail_start = 0;
  if ((((std::uintptr_t)data_x & 0xF) == 0) && (((std::uintptr_t)data_y & 0xF) == 0)) {
    const int vec4_count = sample_size >> 2;
    tail_start = vec4_count << 2;
    const float4* data_x4 = reinterpret_cast<const float4*>(data_x);
    const float4* data_y4 = reinterpret_cast<const float4*>(data_y);

    // Vectorized path: load 4 x/y values per transaction.
    for (int vec4_idx = gid; vec4_idx < vec4_count; vec4_idx += stride) {
      const int base = vec4_idx << 2;
      const float4 xs = data_x4[vec4_idx];
      const float4 ys = data_y4[vec4_idx];

      const float2 p0 = make_float2(xs.x, ys.x);
      const int k0 = find_best_centroid(p0, s_centroid, k);
      labels[base + 0] = k0;
      atomicAdd(s_sum_x + k0, p0.x);
      atomicAdd(s_sum_y + k0, p0.y);
      atomicAdd(s_count + k0, 1);

      const float2 p1 = make_float2(xs.y, ys.y);
      const int k1 = find_best_centroid(p1, s_centroid, k);
      labels[base + 1] = k1;
      atomicAdd(s_sum_x + k1, p1.x);
      atomicAdd(s_sum_y + k1, p1.y);
      atomicAdd(s_count + k1, 1);

      const float2 p2 = make_float2(xs.z, ys.z);
      const int k2 = find_best_centroid(p2, s_centroid, k);
      labels[base + 2] = k2;
      atomicAdd(s_sum_x + k2, p2.x);
      atomicAdd(s_sum_y + k2, p2.y);
      atomicAdd(s_count + k2, 1);

      const float2 p3 = make_float2(xs.w, ys.w);
      const int k3 = find_best_centroid(p3, s_centroid, k);
      labels[base + 3] = k3;
      atomicAdd(s_sum_x + k3, p3.x);
      atomicAdd(s_sum_y + k3, p3.y);
      atomicAdd(s_count + k3, 1);
    }
  }

  // Scalar tail for sizes not divisible by 4.
  for (int i = tail_start + gid; i < sample_size; i += stride) {
    const float2 pt = make_float2(data_x[i], data_y[i]);
    const int best_k = find_best_centroid(pt, s_centroid, k);
    labels[i] = best_k;
