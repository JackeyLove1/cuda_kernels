...
template<const int BM = 32, const int BN = 32, const int BK = 8>
                                             ...
                                             const int K)
{
    ...
    __shared__ float As[BM * BK]; // 32 * 8 = 256
    __shared__ float Bs[BK * BN]; // 8 * 32 = 256

    const int tid = threadIdx.x;
    // 加载 A 的映射: 256个线程对应 32x8 形状
    const int tRowA = tid / BK; // 0..31
    const int tColA = tid % BK; // 0..7

    // 加载 B 的映射: 256个线程对应 8x32 形状
    const int tRowB = tid / BN; // 0..7
    const int tColB = tid % BN; // 0..31

    // 每个线程负责计算 32x32 Tile 中的 4 个元素 (1024 / 256 = 4)
    // 映射方式：thread 0 处理 (0,0), (8,0), (16,0), (24,0) 等
    float accum[4] = {0.f, 0.f, 0.f, 0.f};

    for (int bk = 0; bk < K; bk += BK) {
        // 1. 加载 A 和 B 到共享内存
        As[tid] = A[(bRow * BM + tRowA) * K + bk + tColA];
        Bs[tid] = B[(bk + tRowB) * N + bCol * BN + tColB];

        __syncthreads();

        // 2. 累加计算 (Outer Product 思想)
        int r_base = tid / BN; // 0..7
        int c = tid % BN;      // 0..31

        #pragma unroll
        for (int k = 0; k < BK; k++) {
            float b_val = Bs[k * BN + c];
            accum[0] += As[(r_base + 0) * BK + k] * b_val;
            accum[1] += As[(r_base + 8) * BK + k] * b_val;
            accum[2] += As[(r_base + 16) * BK + k] * b_val;
            accum[3] += As[(r_base + 24) * BK + k] * b_val;
        }
        __syncthreads();
    }
    ...
    int r_base = tid / BN;
    int c = tid % BN;
    C[(bRow * BM + r_base + 0) * N + bCol * BN + c] = accum[0];
    C[(bRow * BM + r_base + 8) * N + bCol * BN + c] = accum[1];
    C[(bRow * BM + r_base + 16) * N + bCol * BN + c] = accum[2];
    C[(bRow * BM + r_base + 24) * N + bCol * BN + c] = accum[3];
}
...
extern "C" void solve(const float *A, const float *B, float *C, int M, int N, int K)
{
    ...
    dim3 blocksPerGrid(CDIV(M, BM), CDIV(N, BN));

    matrix_multiplication_kernel<BM, BN, BK><<<blocksPerGrid, threadsPerBlock>>>(A, B, C, M, N, K);
