...
{
    // 1. 范围限制 (可选，为了极速通常省略或简化)
    // 防止溢出，简单 clamp 一下，例如 [-126, 126]
    x = fminf(fmaxf(x, -126.0f), 126.0f);

    // 2. 分离整数和小数
    // floorf 可能会调用 SFU，这里用类型转换技巧加速
    // 这种 cast 方法假设 x 处于合理范围内
    int i = (int)floorf(x);
    float f = x - (float)i; // f in [0, 1)

    // 3. 多项式拟合 2^f
    // 使用 Remez 算法在 [0,1] 区间拟合 2^x 的 3阶或4阶系数
    // 这里是一个 3阶 近似系数示例
    const float c0 = 1.0f;
    const float c1 = 0.69314718f; // ln(2)
    const float c2 = 0.24022650f; // ln(2)^2 / 2
    const float c3 = 0.05550410f; // ln(2)^3 / 6
    ...
    // 4. 合并整数部分 2^i
    // IEEE 754: float val = 1.mantissa * 2^(exponent - 127)
    // 我们只需要把 p 的指数部分加上 i 即可

    // 把 p 当作整数来处理
    int p_int = __float_as_int(p);

    // 直接在指数位上加 i (指数位在第 23-30 位)
    // 注意：p 的原始范围是 [1, 2)，所以它的原始指数是 127 (二进制 01111111)
    // 我们给它加上 (i << 23)，相当于 exp = 127 + i
    p_int += (i << 23);

    return __int_as_float(p_int);
}
...
/**
Average time: 0.4005 ms
Total time (100 runs): 40.0537 ms
Throughput: 83773.62 million elements/sec
Bandwidth: 670.19 GB/s
Bandwidth breakdown:
  - Read:  335.09 GB/s
  - Write: 335.09 GB/s
 */