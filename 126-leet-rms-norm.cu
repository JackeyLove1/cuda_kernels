...
void __global__ square_sum_kernel(const float* __restrict__ input, float* __restrict__ output,
                                  const int N) {
  const auto tid = threadIdx.x;
...
void __global__ rms_norm_kernel(const float* __restrict__ input, float gamma, float beta,
                                float* __restrict__ output, const int N, const float* square_sum,
                                float eps) {
  const auto tid = threadIdx.x;
  const auto gid = blockIdx.x * blockDim.x + tid;
  const auto stride = blockDim.x * gridDim.x;
  ...
  const int vec_size = N >> 2;
  const int vec_tail = N & 3;
  const float inv_rms = rsqrtf(square_sum[0] / N + eps);
...
extern "C" void solve(const float* input, float gamma, float beta, float* output, int N,
  ...
  float* square_sum;
  cudaMalloc(&square_sum, sizeof(float));
  cudaMemset(square_sum, 0, sizeof(float));
  square_sum_kernel<<<blocksPerGrid, threadsPerBlock>>>(input, square_sum, N);
  rms_norm_kernel<<<blocksPerGrid, threadsPerBlock>>>(input, gamma, beta, output, N, square_sum,
                                                       eps);
