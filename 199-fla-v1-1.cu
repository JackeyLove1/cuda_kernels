...
__global__ void forward_kernel(const float* Q, const float* K, const float* V, const int N,
  ...
  for (int j = 0; j < Tc; j++) {  // Tc * Bc =  N, 扫 K/V 的列块
    ...
    for (int i = 0; i < Tr; i++) {
      ...
      float row_l_new =
          (__expf(row_m_prev - row_m_new) * row_l_prev) + (__expf(row_m - row_m_new) * row_l);
...
torch::Tensor forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
  ...
  forward_kernel<<<grid_dim, block_dim, sram_size>>>(
      Q.data_ptr<float>(), K.data_ptr<float>(), V.data_ptr<float>(), N, d, Tc, Tr, Bc, Br,
      softmax_scale, l.data_ptr<float>(), m.data_ptr<float>(), O.data_ptr<float>());
  return O;
}
