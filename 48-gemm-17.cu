                             ...
                             int          innerColB)
{
    ...
    // Use cp.async for Matrix B
    for (auto offset = 0; offset < BK; offset += rowStrideB) {
        const void* src_ptr = &B[(innerRowB + offset) * N + innerColB * 4];
        void* dst_ptr = &Bs[(innerRowB + offset) * BN + innerColB * 4];
        
        uint32_t smem_ptr;
        asm volatile(
            "{ .reg .u64 asm_dst; cvta.to.shared.u64 asm_dst, %1; cvt.u32.u64 %0, asm_dst; }\n"
            : "=r"(smem_ptr)
            : "l"(dst_ptr)
        );
...
__global__ void __launch_bounds__(NUM_THREADS)
  ...
  for (uint bkIdx = 0; bkIdx < K; bkIdx += 2 * BK) {
    if (doubleBufferIdx == 0) {
      // process current (B0)
      gemm::processFromSmem<BM, BN, BK, WM, WN, WMITER, WNITER, WSUBM, WSUBN, TM,
      ...
      if (bkIdx + BK < K) {
        gemm::processFromSmem<BM, BN, BK, WM, WN, WMITER, WNITER, WSUBM, WSUBN,
