...
/**
Implement batch normalization forward pass for 2D input tensors. Given an input tensor of shape [N,
C] where N is the batch size and C is the number of features, compute the normalized output using
learnable scale (gamma) and shift (beta) parameters. For each feature channel j, batch normalization
computes: μj=1N∑N i=1xi,j σj²=1N∑N i=1(xi,j-μj)² x̂i,j=xi,j-μj/√σj²+ε ŷi,j=γjx̂i,j+βj
 */

static constexpr int TILE = 256;

__global__ void batch_norm_kernel(const float* __restrict__ input, const float* __restrict__ gamma,
                                  const float* __restrict__ beta, float* __restrict__ output,
                                  const int N, const int C, const float eps) {
  const auto tid = threadIdx.x;
  ...
  __shared__ float var_inv;
  for (int row = tid; row < N; row += nthreads) {
    float val = input[row * C + col];
    sum += val;
    smem[row] = val;
  }

  sum = BlockReduceOp<float, SumOp<float>>(sum);
  if (tid == 0) {
    mean = sum / N;
  }
  __syncthreads();
  ...
  for (int row = tid; row < N; row += nthreads) {
    float val = smem[row] - mean;
    local_var = fmaf(val, val, local_var);
  }

  local_var = BlockReduceOp<float, SumOp<float>>(local_var);
  if (tid == 0) {
    var_inv = 1.0f / sqrtf(local_var / N + eps);
  }
  __syncthreads();
  ...
  for (int row = tid; row < N; row += nthreads) {
    float val = smem[row] - mean;
    val = val * var_inv;
    val = fmaf(val, gamma[col], beta[col]);
...
extern "C" void solve(const float* input, const float* gamma, const float* beta, float* output,
  ...
  dim3 nthrs(TILE);
  dim3 nblks(C);
  batch_norm_kernel<<<nblks, nthrs>>>(input, gamma, beta, output, N, C, eps);
