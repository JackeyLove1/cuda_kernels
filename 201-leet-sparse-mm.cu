...
__device__ __forceinline__ void st_global(const float4* ptr, const float4& value) {
  asm volatile("st.global.L1::no_allocate.v4.f32 [%0], {%1, %2, %3, %4};" ::"l"(ptr), "f"(value.x),
               "f"(value.y), "f"(value.z), "f"(value.w));
}

__device__ __forceinline__ void serpentine_mma(float* C, float* A, float* B, int m_val, int n_val) {
#pragma unroll
...
template <int bM, int bN, int bK, int NumThreads, class T, int NumPipe = 1, int base = 0>
__global__ void sgemm_v4(const T* __restrict__ A, const T* __restrict__ B, float* __restrict__ C,
                         int M, int N, int K) {
  const int tid = threadIdx.x;
  const int warp_id = tid / 32;
  const int lane_id = tid % 32;
...
#pragma unroll
      ...
      reinterpret_cast<float4*>(reg_a)[reg_next_idx * 2 + 0] =
          reinterpret_cast<float4*>(tile_sA + tk_next * bM_pad + row_offset)[0];
      reinterpret_cast<float4*>(reg_a)[reg_next_idx * 2 + 1] =
          reinterpret_cast<float4*>(tile_sA + tk_next * bM_pad + row_offset)[1];
...
#pragma unroll
      ...
      // reinterpret_cast<float4 *>(reg_b)[reg_next_idx * 4 + 0] = reinterpret_cast<float4
      // *>(tile_sB + tk_next * bN_pad + col_offset1)[0]; reinterpret_cast<float4
      // *>(reg_b)[reg_next_idx * 4 + 1] = reinterpret_cast<float4 *>(tile_sB + tk_next * bN_pad +
      // col_offset1)[1]; reinterpret_cast<float4 *>(reg_b)[reg_next_idx * 4 + 2] =
      // reinterpret_cast<float4 *>(tile_sB + tk_next * bN_pad + col_offset2)[0];
      // reinterpret_cast<float4 *>(reg_b)[reg_next_idx * 4 + 3] = reinterpret_cast<float4
      // *>(tile_sB + tk_next * bN_pad + col_offset2)[1];
...
#pragma unroll
    for (int j = 0; j < n_val / 4; ++j) {
      ...
      // gC[local_row * N + local_col] = reg_c[i * n_val + j];
      reinterpret_cast<float4*>(shared_memory + local_row * bN + local_col)[0] =
          reinterpret_cast<float4*>(reg_c + i * n_val)[j];  // reuse shared memory
    }
  }
  __syncthreads();
