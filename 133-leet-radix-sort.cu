...
/**
Implement a radix sort algorithm that sorts an array of 32-bit unsigned integers on a GPU. The
program should take an input array of unsigned integers and sort them in ascending order using the
radix sort algorithm. The input parameter contains the unsorted array, and the sorted result should
be stored in the output array.
 */

// input, output are device pointers
extern "C" void solve(const unsigned int* input, unsigned int* output, int N) {
  thrust::device_ptr<const unsigned int> input_ptr = thrust::device_pointer_cast(input);
  thrust::device_ptr<unsigned int> output_ptr = thrust::device_pointer_cast(output);
  thrust::copy(input_ptr, input_ptr + N, output_ptr);
  thrust::sort(output_ptr, output_ptr + N, thrust::less<unsigned int>());
}
