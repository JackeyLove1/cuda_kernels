 ...
 *
 * 三条优化路径：
 *  1. Warp-level Bitonic Sort  —— K ≤ 32，纯寄存器，零 shared memory
 *  2. Block-level Min-Heap     —— K ≤ 1024，shared memory 维护堆
 *  3. CUB DeviceRadixSort      —— 大 K / 大 N 通用高性能路径
 *
 * 编译：nvcc -O3 -arch=sm_80 -std=c++17 topk.cu -o topk
 */
...
__launch_bounds__(WARPS_PER_CTA* WARP_SIZE) __global__
    void topkGatingSoftmax(const T* input, const bool* finished, float* output, const int num_rows,
                           int* indices, const int k, const int start_expert, const int end_expert,
                           const bool renormalize, const float moe_softcapping,
  ...
  // We finally start setting up the read pointers for each thread. First, each thread jumps to the
  // start of the row it will read.
  const T* thread_row_ptr = input + thread_row * ELTS_PER_ROW;

  // Now, we compute the group each thread belong to in order to determine the first column to start
  // loads.
...
// Now, we find the max within the thread group and distribute among the threads. We use a butterfly
// reduce. lane id: 0-31 within a warp
#pragma unroll
  for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2) {
    // butterfly reduce with (lane id ^ mask)
    thread_max =
        max(thread_max, SGLANG_SHFL_XOR_SYNC_WIDTH(0xffffffff, thread_max, mask, THREADS_PER_ROW));
  }

  // From this point, thread max in all the threads have the max within the row.
  // Now, we subtract the max from each element in the thread and take the exp. We also compute the
  // thread local sum.
  float row_sum = 0;
#pragma unroll
  for (int ii = 0; ii < VPT; ++ii) {
...
// Now, we perform the sum reduce within each thread group. Similar to the max reduce, we use a
// bufferfly pattern.
#pragma unroll
  for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2) {
  ...
  // From this point, all threads have the max and the sum for their rows in the thread_max and
  // thread_sum variables respectively. Finally, we can scale the rows for the softmax. Technically,
  // for top-k gating we don't need to compute the entire softmax row. We can likely look at the
  // maxes and only compute for the top-k values in the row. However, this kernel will likely not be
  // a bottle neck and it seems better to closer match torch and find the argmax after computing the
  // softmax.
...
#pragma unroll
  ...
  // Now, softmax_res contains the softmax of the row chunk. Now, I want to find the topk elements
  // in each row, along with the max index.
...
#pragma unroll
      for (int ii = 0; ii < ELTS_PER_LDG; ++ii) {
        ...
        // No check on the experts here since columns with the smallest index are processed first
        // and only updated if > (not >=)
        if (val > max_val) {
          max_val = val;
          expert = col + ii;
        }
...
// Now, we perform the argmax reduce. We use the butterfly pattern so threads reach consensus about
// the max. This will be useful for K > 1 so that the threads can agree on "who" had the max value.
// That thread can then blank out their max with -inf and the warp can run more iterations...
#pragma unroll
    for (int mask = THREADS_PER_ROW / 2; mask > 0; mask /= 2) {
