...
extern "C" void solve(const float* d_matrix, float* d_result, int N, int P) {
  ...
  cuda_eye_fast(d_result, N, N);
  float* d_temp = nullptr;
  cudaMalloc(&d_temp, bytes);
  float* d_cur = d_result;
  float* d_next = d_temp;
  for (int i = 0; i < P; ++i) {
    matrix_multiplication_kernel<<<gridDim, blockDim>>>(d_cur, d_matrix, d_next, N, N, N);
    std::swap(d_cur, d_next);
  }
  if (d_cur != d_result) {
    cudaMemcpy(d_result, d_cur, bytes, cudaMemcpyDeviceToDevice);
  }
