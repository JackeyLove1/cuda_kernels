...
gemm_multi_stage(void *Dptr, const void *Aptr, const void *Bptr, int m, int n,
  ...
  T *Ashm = shm_data;
  T *Bshm = shm_data + cosize_v<SmemLayoutA>;
  ...
  // it is like: auto tCrD = make_fragment_like(thr_mma.partition_C(gD));
...
struct GemmConfig {
  ...
  static constexpr int shm_size_AB =
      cosize_v<SmemLayoutA> + cosize_v<SmemLayoutB>;
  static constexpr int shm_size_C = cosize_v<SmemLayoutC>;
